...
<html lang="en">
  ...
  >
    ...
    <div
      id="gameWrapper"
      class="opacity-0 transition-opacity duration-300 max-w-6xl mx-auto px-4 py-8 md:py-12"
    >
      <!-- Header -->
      <header
        class="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-4 mb-6 md:mb-8"
      >
        <div>
    ...
    >
      <div
        class="modal-show w-full max-w-lg rounded-3xl bg-slate-900 border border-slate-700 shadow-2xl"
      >
        <div class="p-6 md:p-8">
          <div class="flex items-start justify-between gap-4">
            ...
            <button
              id="closeModal"
              class="p-2 rounded-xl bg-slate-800 hover:bg-slate-700"
            >
              ✕
            </button>
          </div>
    ...
    <script>
      ...
      function buildRound() {
        ...
        for (let i = 0; i < CORRECT_COUNT; i++) {
          ...
          gameData.push({ text: col.name, ink: col.value, isCorrect: true });
        }
        ...
        for (let i = CORRECT_COUNT; i < WORD_COUNT; i++) {
          ...
          gameData.push({ text: txt.name, ink: ink.value, isCorrect: false });
        }
      ...
      function startTimer() {
        ...
        const updateTimer = () => {
          remainingTime = GAME_TIME - (Date.now() - startTime);
          if (remainingTime <= 0) {
            timerEl.textContent = "00:00";
            submit();
            return;
          }
          timerEl.textContent = fmt(remainingTime);
        };
      ...
      function submit() {
        ...
        rScoreText.textContent =
          `Score: ${correctSel} / ${totalCorrect} correct` +
          (wrongSel > 0 ? ` • ${wrongSel} wrong selections` : "");
