...
<body class="bg-gradient-to-br from-game-dark via-game-blue to-game-purple min-h-screen flex flex-col overflow-hidden">
            ...
            <div class="flex justify-between items-center py-2">
                ...
                <div class="text-sm">
                    <a href="{{ route('user.signup') }}"
                        class="bg-white/10 px-3 py-2 md:px-4 md:py-2 rounded-lg hover:bg-white/20 transition-all duration-300 text-sm md:text-base">
                        Sign Up
                    </a>
                </div>
    ...
    <main class="flex-1 flex items-center justify-center px-4 py-6">
        <div
            class="relative max-w-md w-full bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-6 md:p-8 space-y-6 animate-slideUp overflow-hidden">

            <!-- Floating Orbs (Animated Background) -->
            <div
                class="absolute top-[-20px] left-[-20px] md:top-[-30px] md:left-[-30px] w-12 h-12 md:w-16 md:h-16 bg-blue-500/50 rounded-full animate-float blur-2xl">
            </div>
            <div
                class="absolute bottom-[-30px] right-[-30px] md:bottom-[-40px] md:right-[-40px] w-20 h-20 md:w-24 md:h-24 bg-purple-500/40 rounded-full animate-float blur-3xl">
            </div>
            ...
            <form id="loginForm" action="{{ route('user.authenticate') }}" method="POST" class="space-y-4">
                ...
                <p class="text-center text-gray-200 text-sm">
                    ...
                    <a href="{{ route('user.signup') }}" class="text-purple-300 hover:text-purple-400 font-semibold">
                        Sign Up
                    </a>
                </p>
            </form>
